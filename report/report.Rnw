...
\section*{Methods}
  ...
  Based on these dates, the tagging data provide
  \Sexpr{format(floor(sum(info$lastDay)),big.mark=',')} turtle-days of tracking
  ...
  We believe that most premature releases and tag losses are due to tag or attachment failure
