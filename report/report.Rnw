...
\section*{Results}
  Over the \Sexpr{length(unique(year(info$deployDate)))} tagging-seasons, tags were deployed on \Sexpr{nrow(info)} turtles and
  \Sexpr{format(floor(sum(info$lastDay)),big.mark=',')} turtle-days of tracking data was collected
