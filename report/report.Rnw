...
plotFunc<-function(km,col='black',text='',ylab='Proportion surviving',legendPos='bottomright'){
  par(mar=c(2.6,3,.1,.1))
  plot(km,xlab='',ylab=ylab,conf.int=FALSE,mark.time=TRUE,mgp=c(2.1,.6,0),las=1,tcl=-.4,col=col,lwd=2)
...
Combined these tracking data cover
\Sexpr{floor(sum(info$lastDay))} turtle-days (Figure \ref{figMinMax}).
...
Tag data was [[MIKE: DOWNLOADED FROM WC WEBSITE]] and processed using WC-DAP v3.0.369.
Status messages and the minimum and maximum depths reported by WC-DAP were used to infer turtle fate.
...
reaching a maximum depth threshold of 1700 meters [[MIKE: Did 2015 tags have this threshold? I don't see annotated anywhere?]].
...
@
  ...
  \caption{Kaplan-Meier curves showing the estimated proportion of turtles surviving over time for all turtles (A); deep, light or no hook turtles (B); observer or fisherman tagged turtles (C) and turtles with long ($>30$ cm) or short pieces of monofilament left attached to hooks (D). Shaded regions indicate 95\% confidence intervals.}
  \label{figKms}
\end{figure}
\begin{figure}
\centering
...
<<echo=FALSE,fig=TRUE,width=3.2,height=3.2>>=
  ...
  plotFunc(km,yearColor,text='C',legendPos='topright')
...
<<echo=FALSE,fig=TRUE,width=3.2,height=3.2>>=
  ...
  yearColor<-rainbow.lab(length(unique(years)),lightScale=0,lightMultiple=.7,alpha=1,start=1,end=-1)
  yearColor2<-rainbow.lab(length(unique(years)),lightScale=0,lightMultiple=.7,alpha=.1,start=1,end=-1)
  yearColor3<-rainbow.lab(length(unique(years)),lightScale=0,lightMultiple=.7,alpha=.5,start=1,end=-1)
  ...
  plotFunc(km,yearColor,text='B',legendPos='topright')
  addConfInt(km,yearColor2,yearColor3,lty=2)
@
} \subfloat{
<<echo=FALSE,fig=TRUE,width=3.2,height=3.2>>=
  ...
  names(yearColor)<-names(yearColor2)<-names(yearColor3)<-unique(obs)
  km<-survfit(drop~obs,conf.type='log-log')
  plotFunc(km,yearColor,text='D',legendPos='topright')
  addConfInt(km,yearColor2,yearColor3,lty=2)
@
}

  \caption{Kaplan-Meier curves showing the estimated proportion of tags remaining on turtles. Maximum depth, constant depth and scheduled released are counted as censored observations.}
  \label{figDrop}
\end{figure}
...
@
  ...
  \caption{Minimum and maximum depth reports for all turtles in the study}
  \label{figMinMax}
\end{figure}
...
@
  ...
  \caption{Minimum and maximum depth reports for all turtles in the study in the month prior to release}
  \label{figRelease}
\end{figure}
