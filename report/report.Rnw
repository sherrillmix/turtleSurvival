...
@
  ...
  \caption{Kaplan-Meier curves showing the estimated proportion of tags remaining on turtles for all turtles (A), turtles split by tagging by an observer or a fisherman (B), turtles split by tagging year (C) and turtles split by tag type (D). Note that tagging year and tag type are highly correlated with the majority of tags in 2012 being PAT, in 2014 being SPLASH and 2015 being MinPat. A single tag deployed in 2011 is not shown in (C). Maximum depth, constant depth and scheduled released are counted as censored observations (crosses).}
  \label{figDrop}
\end{figure}
