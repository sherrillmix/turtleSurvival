...
\section*{Results}
  ...
  \subsection*{Tag failures}
    ...
    so precisely defining which variable is affecting performance is difficult
    ...
    \Sexpr{round(100-summary(dropKmType,time=30)$lower[names(dropKmType$strata)=='types=SPLASH']*100,1)}\%) of SPLASH tags failing within 30 days of deployment.
