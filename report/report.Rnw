...
\section*{Methods}
  ...
  We estimated survival rates using Kaplan-Meier estimates \citep{Kaplan1958} and compared between conditions using Cox proportional hazards models \citep{Cox1972}.
...
\section*{Results}
  ...
  \Sexpr{round(summary(km,time=365)$surv*100,1)}\%
  (95\% CI: \Sexpr{round(summary(km,time=365)$lower*100,1)}--%
  \Sexpr{round(summary(km,time=365)$upper*100,1)}\%)
  of turtles survived at least 365 days after tagging
  (Figure \ref{figKms}A).
  ...
  Tag type and tagging year were highly correlated (Table \ref{tabTagYear}) so precisely defining which variable is affecting performance is difficult but SPLASH tags deployed in 2014 had a very high failure rate (Figure \ref{figDrop}CD).
...
further tagging is necessary if we wish to determine variables correlated with mortality.
