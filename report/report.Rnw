...
\section*{Methods}
  ...
  %\citep{Kaplan1958}
  We estimated survival rates using Kaplan-Meier estimates and compared between conditions using Cox proportional hazards models.
...
\section*{Results}
  ...
  \Sexpr{round(summary(km,time=90)$upper*100,1)}\%) of turtles survived at least 90 days after tagging and
  \Sexpr{round(summary(km,time=365)$surv*100,1)}\%
  (95\% CI: \Sexpr{round(summary(km,time=365)$lower*100,1)}--%
  \Sexpr{round(summary(km,time=365)$upper*100,1)}\%) of turtles survived at least 365 days after tagging (Figure \ref{figKms}A).

  Hooking location did not appear to have a significant effect on mortalities
  (Cox proportional hazard model: $p=\Sexpr{round(coef(summary(coxHook))[,'Pr(>|z|)'],2)}$).
  ...
  ($p=\Sexpr{round(coef(summary(coxObs))[,'Pr(>|z|)'],2)}$)
  (Figure \ref{figKms}C).
