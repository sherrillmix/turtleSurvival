...
\section{Methods}
  ...
  To infer the final time when the data clearly indicated the tag was on the turtle,
  ...
  Turtle fate was determined based on the tag release condition with tag released due to extreme depth or constant depth counted as mortalities and all other releases counted as censored data where the turtle was apparently alive when the tag was released. A limitation of these data, is that there is little opportunity to identify a sudden mortality event, e.g. a large shark bite or propeller strike, which both kills the turtle and releases the tag. We believe that most premature releases are due to tag attachment failure due to a particularly release-prone batch of MinPat tags in 2014 (Fig. \ref{figDrop}C,D).
